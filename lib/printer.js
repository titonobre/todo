...
function formatComposedState(state, teamState) {
  if (state === teamState) return formatState(state);
...
function printTasks(tasks, { showDescription }) {
  ...
  tasks
    .map(({ id, title, description, url, state, teamState, assignees, progress }) => ({
      id: chalk.red.bold(id),
      title,
      description: description && showDescription && formatHtmlText(description),
      url: chalk.blue(url),
      state: formatComposedState(state, teamState),
      assignees: formatAssignees(assignees),
      progress: formatProgress(progress),
    }))
    .forEach(({ id, title, description, url, state, teamState, assignees, progress }) => {
